...
local augroup = api.nvim_create_augroup("TwelveFloatTerm", { clear = true })
...
local function apply_window_options(win)
  ...
  wo.scrolloff = 0
  wo.sidescrolloff = 0
  api.nvim_set_option_value("winhighlight", "NormalFloat:Normal,FloatBorder:FloatBorder", { win = win })
end

local function create_floating_window(opts)
  opts = opts or {}
  local geom = compute_geometry(opts)
  local buf = opts.buf and api.nvim_buf_is_valid(opts.buf) and opts.buf or api.nvim_create_buf(false, true)
...
local function resize_floating_window()
  ...
  api.nvim_win_set_config(state.floating.win, {
    relative = "editor",
    width = geom.width,
    height = geom.height,
    col = geom.col,
...
local function toggle_terminal()
  ...
  if not api.nvim_win_is_valid(state.floating.win) then
    ...
    pcall(api.nvim_win_hide, state.floating.win)
    state.floating.win = -1
  end
end
