...
map("n", "<leader>l", ":PackStatus<CR>", { desc = "Show pack plugin status", silent = true })
...
map("n", "-", "<cmd>Oil --preview<CR>", { desc = "Open file explorer" })
map("i", "jk", "<Esc>", opts)
...
map("n", "<leader>lr", function()
  ...
  vim.defer_fn(function()
    ...
    vim.notify(("Restarted: %s"):format(table.concat(names, ", ")), vim.log.levels.INFO, { title = "twelve.lsp" })
  end, 100)
